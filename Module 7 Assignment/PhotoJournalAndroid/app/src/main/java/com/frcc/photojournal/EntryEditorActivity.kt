...
//
//  FIX 2 (Module 7): Replaced raw Thread{} + runOnUiThread with a
//              single-thread Executor + main-thread Handler.  The
//              executor is shut down in onDestroy() so background
//              work is cancelled when the activity is destroyed,
//              preventing memory leaks and stale UI updates.
// ============================================================
...
class EntryEditorActivity : AppCompatActivity() {
    ...
    // FIX 2: Executor + Handler instead of raw Thread{} — lifecycle-safe
    ...
    private fun displayPhoto() {
        ...
        val pathSnapshot = photoPath          // capture to avoid race on rotation
        // FIX 2: Use managed executor so the task is cancelled in onDestroy()
        photoExecutor.execute {
            val bmp = decodeSampledBitmap(pathSnapshot, 1080, 1080)
            mainHandler.post {
                // Only update if this path is still the current one
                if (photoPath == pathSnapshot) {
    ...
    override fun onDestroy() {
        super.onDestroy()
        photoExecutor.shutdownNow()   // FIX 2: cancel pending background work
    }
